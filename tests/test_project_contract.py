...
class ProjectContractTests(unittest.TestCase):
    ...
    def test_local_surface_rebuild_is_feature_locked_and_recoverable(self):
        ...
        self.assertIn('canvas_macro_folds if mode in {"canvas", "canvas_multifold", "canvas_physics"}', adapter)
        ...
        self.assertIn('"fair_base_then_source_aligned_wave_outer_envelope_v3"', adapter)
        self.assertIn('"canvas_multifold", "多折面帆布优化"', operators)
        self.assertIn('text="多折面帆布优化"', panel)
        self.assertIn('"canvas_multifold": "确认多折面帆布并清除标记"', panel)
        self.assertIn('def _multifold_canvas_edges', adapter)
        self.assertIn('def _multifold_attachment_points', adapter)
        self.assertIn('def _multifold_chart_frame', adapter)
        self.assertIn('def _multifold_safe_fraction', adapter)
        self.assertIn('def _fair_multifold_canvas_microfacets', adapter)
        self.assertIn('"source_normal_bilateral_microfacet_fairing_v1"', adapter)
        self.assertIn('"fold_preserving_microfacet_fairing_attachment_wrinkle_outer_envelope_v2"', adapter)
        self.assertIn('smooth_preview=mode != "canvas_multifold"', adapter)
        self.assertIn('face.smooth = mode in {"canvas", "canvas_physics"}', adapter)
        self.assertIn('"denser_near_hanging_attachment_then_fade_inward"', adapter)
        self.assertIn('"canvas_physics", "物理帆布重建（绳挂）"', operators)
        self.assertIn('text="物理帆布重建（绳挂）"', panel)
        self.assertIn('"canvas_physics": "确认物理帆布并清除标记"', panel)
